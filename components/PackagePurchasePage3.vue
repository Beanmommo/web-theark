...
import { type PackageDetails, PaymentMethods, type Invoice, InvoiceType } from '../types/data';
...
async function clickHandlerSubmit()
{
  ...
  if (paymentMethod.value === PaymentMethods.CREDIT_CARD)
  {
    ...
    const creditPackageKey = await creditsStore.addCreditPackage(finalInvoiceData, props.packageItem)
    if (creditPackageKey)
      await invoicesStore.updateInvoiceCreditPackageKey(finalInvoiceData.id, creditPackageKey)
...
<template>
  <div class="packagePurchasePage3">
    ...
    <PackagePurchasePage3Details :packageItem="packageItem" v-if="packageItem" />
    <UserDetailsForm />
    ...
    <div class="buttons">
      <Button :disabled="!formCompleted || loading" @click="clickHandlerSubmit">Confirm My Purchase</Button>
