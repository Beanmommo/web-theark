...
await Promise.all([useAsyncData(() => packagesStore.fetchPackages())]);
...
onMounted(() => {
  if (route.query.package) initialisePackage(route.query.package as string);
});
...
function initialisePackage(packageId: string) {
  selectedPackage.value = packageConfig.value.find(
    (packageConfig) => packageConfig.id === packageId
  );
...
<template>
  ...
  <PackagePurchasePage3
    v-if="isPaymentPage && selectedPackage"
    @back="backHandler"
